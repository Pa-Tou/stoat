...
TEST_CASE("Output loop with snarl", "[changeref]") {
    /*
              --------
             |   2    |
             \ / \    /
...
TEST_CASE("Output loop with snarl with bgzipped output", "[changeref]") {
    /*
              --------
             |   2    |
             \ / \    /
    ...
    SECTION("Change ref to same path") {
        ...
        REQUIRE(std::filesystem::exists(output_dir+"/stoat.assoc.pvalues.path0.tsv.gz"));

        // Unzip the outputs
        std::string unzip_cmd = "gunzip " + output_dir+"/stoat.assoc.pvalues.path0.tsv.gz";
        command_output = std::system(unzip_cmd.c_str());
        unzip_cmd = "gunzip " + output_dir+"/stoat.assoc.pvalues.tsv.gz";
        command_output = std::system(unzip_cmd.c_str());
    ...
    SECTION("Change ref to path1") {
        ...
        REQUIRE(std::filesystem::exists(output_dir+"/stoat.assoc.pvalues.path1.tsv.gz"));

        // Unzip the outputs
        std::string unzip_cmd = "gunzip " + output_dir+"/stoat.assoc.pvalues.path1.tsv.gz";
        command_output = std::system(unzip_cmd.c_str());
        unzip_cmd = "gunzip " + output_dir+"/stoat.assoc.pvalues.tsv.gz";
        command_output = std::system(unzip_cmd.c_str());
