...
TEST_CASE("Node vcf on simple bubble", "[node]") {
    /*
           1    
         /   \
        0--2--4
    ...
    paths are called pathN
    {0, 1, 4}, {0, 1, 4}, {0, 2, 4}, {0, 3, 4}
    */
    ...
    vcf_out << "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\tS1\tS2" << std::endl;
    vcf_out << "path0\t0\t>1>5\tA\tG,C\t60\t.\tLV=0;AT=>1>2>5,>1>3>5,>1>4>5\tGT\t0/1\t0/2" << std::endl;
    vcf_out.close();
    ...
    SECTION("Test without untangling") {
        ...
        std::string cmd = (std::string)"../bin/stoat vcf --mode node -u"
            + " -g " + graph_base + ".hg"
            + " -d " + graph_base + ".dist"
            + " -R " + reference_filename
            + " -v " + vcf_filename
        ...
        stoat::BgzReader gt_reader(output_dir + "/node_info.tsv");
        std::string line; 
        // Get the index of each sample in the vcf (S1#0, S1#1, S2#0, S2#1)
        std::vector<size_t> sample_index(4,0);
        while (gt_reader.getline(line)) {
            if (line.at(0) == '#') {
                ...
                if (std::mismatch(header.begin(), header.end(), line.begin()).first == header.end() ) {
                    ...
                    std::string item;
                    std::getline(headerstream, item, '\t'); // got ref
                    std::getline(headerstream, item, '\t'); // got pos
                    std::getline(headerstream, item, '\t'); // got depth
            ...
            std::stringstream linestream(line);
            std::string node_id;
            std::getline(linestream, node_id, '\t');
            // Get to the 5th item, which is the first genotype
            std::string item;
            std::getline(linestream, item, '\t'); // got ref
            std::getline(linestream, item, '\t'); // got offset
            std::getline(linestream, item, '\t'); // got depth
            ...
            if (node_id == "1") {
                ...
                // Get the genotypes
                REQUIRE(genotypes[sample_index[0]] == "1");
                REQUIRE(genotypes[sample_index[1]] == "1");
                REQUIRE(genotypes[sample_index[2]] == "1");
                REQUIRE(genotypes[sample_index[3]] == "1");

            } else if (node_id == "2") {

                // Get the genotypes
                REQUIRE(genotypes[sample_index[0]] == "1");
                REQUIRE(genotypes[sample_index[1]] == "0");
                REQUIRE(genotypes[sample_index[2]] == "1");
                REQUIRE(genotypes[sample_index[3]] == "0");

            } else if (node_id == "3") {

                // Get the genotypes
                REQUIRE(genotypes[sample_index[0]] == "0");
                REQUIRE(genotypes[sample_index[1]] == "1");
                REQUIRE(genotypes[sample_index[2]] == "0");
                REQUIRE(genotypes[sample_index[3]] == "0");
                ...
                // Snarl that shouldn't exist
                std::cerr << "Node that shouldn't exist " << node_id << std::endl;
                REQUIRE(false);
            }
        }
