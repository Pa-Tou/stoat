...
for (i in seq_len(n_sims)) {
  ...
  beta_true <- c(runif(1, -2, 2), rep(0, k - 2))  # true effect sizes on the first SNP only
...
for (rep in 1:n_reps) {
  ...
  # ---- Generate Y ----
  beta_true <- c(runif(1, -0.5, 0.5), rep(0, ncol(X) - 1))
  intercept <- 1
  Y <- intercept + X %*% beta_true + rnorm(n, sd = 0.1)
  Y <- as.numeric(Y)

  # non-significant regression
  beta_null <- rep(0, ncol(X))   # length 2
  Y_null <- intercept + X %*% beta_null + rnorm(n, sd = 0.5)
  Y_null <- as.numeric(Y_null)
