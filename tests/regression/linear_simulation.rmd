...
# Fit linear model with perfectly colliPredictors
model2 <- lm(Y ~ X1 + X2)
summary(model2)
```
...
1. **STOAT Implementation**  
   - Uses `eigen` and Boost libraries for robust computation of the inverse of \(X^TX\).  
   - Designed for numerical stability and efficiency on large datasets.  
   - Particularly robust against collinearity and near-singular matrices.
...
3. **RVTEST OLS Implementation**  
   - A specialized OLS implementation often used in genomics and high-dimensional data.  
   - Optimized for large-scale regression tasks, potentially with additional model diagnostics.  
   - Offers an alternative reference for coefficient estimates and p-values.

These three implementations are compared against R’s standard `lm()` function, which serves as the "ground truth" for this analysis. The comparison focuses on:

- **coefficients**: checking if the estimates are consistent across methods.
- **p-values**: evaluating statistical significance and comparing results across implementations.
- **robustness to collinearity**: assessing how each method handles near-singular matrices.
...
set.seed(123)
n_sims <- 10000 # number of simulation
n <- 200 # sample number
...
run_linear_simulation <- function(n_sims, n, k, beta_1, sigma,
  ...
  if (collinearity) {
    ...
    # Determine number of last rows (between 1 and 10% of n)
    number_last_element <- 100
    start_row <- n - number_last_element + 1
    
    # Set selected rows to 0
    X[start_row:n, ] <- 0
    
    # Set last two columns to 0.5
    last_cols <- (ncol(X)-1):ncol(X)
    X[start_row:n, last_cols] <- 0.5
    
    # Change one specific row to 0,1 in the last two columns
    # specific_row <- start_row  # for example, the first row of the modified block
    # X[specific_row, last_cols] <- c(0, 1)
...
plot_linear_simulation_results <- function(results_result_all, null = FALSE, title_prefix = "") {
  ...
  if (null) {
    ...
    plots$pval_hist_small <- ggplot(df_pvals_long_small, aes(x = PValue, fill = Method)) +
      geom_histogram(alpha = 0.5, position = "dodge", bins = 100) +
      theme_bw() +
      labs(title = paste("P-value Distributions (0 to 0.000001)", title_prefix),
          x = "P-value", y = "Frequency")
  }
...
## All variant plot significant

```{r plot1, echo=FALSE, message=FALSE, warning=FALSE}
results_all_significant <- run_linear_simulation(n_sims, n, k, beta_1, sigma, significant=TRUE, collinearity=FALSE, merge=FALSE)
plots_all_significant <- plot_linear_simulation_results(results=results_all_significant, null=FALSE, title_prefix = "[All variant type + Significant]")
plots_all_significant
```

## All variant plot no significant

```{r plot2, echo=FALSE, message=FALSE, warning=FALSE}
results_all_no_significant <- run_linear_simulation(n_sims, n, k, beta_1, sigma, significant=FALSE, collinearity=FALSE, merge=FALSE)
plots_all_no_sig <- plot_linear_simulation_results(results_all_no_significant, null = TRUE, title_prefix = "[All variant type + No Significant]")
plots_all_no_sig
```

## Perfect collinearity without merging significant

```{r plot3, echo=FALSE, message=FALSE, warning=FALSE}
results_colinearity_significant <- run_linear_simulation(n_sims, n, k, beta_1, sigma, significant=TRUE, collinearity=TRUE, merge=FALSE)
plots_colinearity_significant <- plot_linear_simulation_results(results_colinearity_significant, null = FALSE, title_prefix = "[Collinear + no merged + Significant]")
plots_colinearity_significant
```

## Perfect collinearity without merging no significant

```{r plot4, echo=FALSE, message=FALSE, warning=FALSE}
results_colinearity_no_significant <- run_linear_simulation(n_sims, n, k, beta_1, sigma, significant=FALSE, collinearity=TRUE, merge=FALSE)
plots_colinearity_no_significant <- plot_linear_simulation_results(results_colinearity_no_significant, null = TRUE, title_prefix = "[Collinear + no merged + No Significant]")
plots_colinearity_no_significant
```

## Perfect collinearity with merging significant

```{r plot5, echo=FALSE, message=FALSE, warning=FALSE}
results_colinearity_significant_merge <- run_linear_simulation(n_sims, n, k, beta_1, sigma, significant=TRUE, collinearity=TRUE, merge=TRUE)
plots_colinearity_significant_merge <- plot_linear_simulation_results(results_colinearity_significant_merge, null = FALSE, title_prefix = "[Collinear + merged + Significant]")
plots_colinearity_significant_merge
```

## Perfect collinearity with merging no significant

```{r plot6, echo=FALSE, message=FALSE, warning=FALSE}
results_colinearity_no_significant_merge <- run_linear_simulation(n_sims, n, k, beta_1, sigma, significant=FALSE, collinearity=TRUE, merge=TRUE)
plots_colinearity_no_significant_merge <- plot_linear_simulation_results(results_colinearity_no_significant_merge, null = TRUE, title_prefix = "[Collinear + merged + No Significant]")
plots_colinearity_no_significant_merge
```
