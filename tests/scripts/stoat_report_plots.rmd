...
manhattan_plot(df_binary_vcf, subtitle="on Binary [stoat vcf]", "P_CHI2", "P_FISHER", "P_ADJUSTED")
```
...
```{r binary-graph}
# Make sure SNARL is character
df_binary_vcf$SNARL <- as.character(df_binary_vcf$SNARL)
df_binary_graph$SNARL <- as.character(df_binary_graph$SNARL)

# Full join by SNARL
df_joined <- full_join(df1, df2, by = "SNARL", suffix = c("_df1", "_df2"))

# Compare columns for matching SNARL
diff_summary <- df_joined %>%
  filter(!is.na(START_POS_df1) & !is.na(START_POS_df2)) %>%  # only SNARL in both
  rowwise() %>%
  mutate(
    diff_cols = list(names(c_across(ends_with("_df1"))) %>%
                       sapply(function(col) {
                         col2 <- sub("_df1$", "_df2", col)
                         val1 <- get(col)
                         val2 <- get(col2)
                         if(is.na(val1) & is.na(val2)) FALSE
                         else if(is.na(val1) | is.na(val2)) TRUE
                         else val1 != val2
                       }) %>% which() %>% names()),
    num_diff = length(diff_cols)
  ) %>%
  select(SNARL, num_diff, diff_cols)

# Print summary
cat("Number of differing columns per SNARL:\n")
print(diff_summary)
