...
* **Stoat Graph**:
  ...
  * Fixed bug with distanceless index
  * Added smaller curated tests for output
...
* **Stoat**:
  ...
  * Make an R stoat library for plots.
  * Update the simulation graph script.

* **Stoat VCF**:

  * Test all methods with better simulation graph.
  * Use FID (family information).

* **Stoat Graph**:

  * Fix bug finding offsets of nodes in paths (probably `vg` bug).
  * Make input samples file match Stoat VCF.
...
```{r stoat-lauching, message=FALSE, warning=FALSE, echo=FALSE}
# # BINARY
# system("stoat vcf -p data/binary/pg.full.pg -d data/binary/pg.full.dist -r data/binary/pg.chromosome -v data/binary/merged_output.vcf.gz -b data/binary/phenotype.tsv --output output_binary")

# # BINARY + COVARIATE
# system("stoat vcf -p data/binary/pg.full.pg -d data/binary/pg.full.dist -r data/binary/pg.chromosome -v data/binary/merged_output.vcf.gz -b data/binary/phenotype.tsv --covariate data/binary/covariate.tsv --covar-name PC1,SEX,PC3 --output output_binary_covar")

# # QUANTITATIVE
# system("stoat vcf -p data/quantitative/pg.full.pg -d data/quantitative/pg.full.dist -r data/quantitative/pg.chromosome -v data/quantitative/merged_output.vcf.gz -q data/quantitative/phenotype.tsv --output output_quantitative")

# # QUANTITATIVE + COVARIATE
# system("stoat vcf -p data/quantitative/pg.full.pg -d data/quantitative/pg.full.dist -r data/quantitative/pg.chromosome -v data/quantitative/merged_output.vcf.gz -q data/quantitative/phenotype.tsv --covariate data/quantitative/covariate.tsv --covar-name PC1,SEX,PC3 --output output_quantitative_covar")

# # EQTL
# system("stoat vcf -s data/eqtl/snarl_analyse.tsv -v data/eqtl/merged_output.vcf.gz -e data/eqtl/qtl.tsv --gene-position data/eqtl/gene_position.tsv --output output_eqtl")

# # EQTL + COVARIATE
# system("stoat vcf -s data/eqtl/snarl_analyse.tsv -v data/eqtl/merged_output.vcf.gz -e data/eqtl/qtl.tsv --gene-position data/eqtl/gene_position.tsv --covariate data/eqtl/covariate.tsv --covar-name SEX,PC1,PC2,PC3 --output output_eqtl_covar")

# # BINARY DECONSTRUCT
# system("stoat vcf -p ../data/binary/pg.full.pg -d ../data/binary/pg.full.dist -r ../data/binary/pg.chromosome -v ../data/binary/pg.full.deconstruct.change2.vcf -b ../data/binary/phenotype.tsv --output ../output_binary_deconstruct")

# # BINARY GRAPH
# system("stoat graph -g data/binary/pg.full.pg -d data/binary/pg.full.dist -T chi2 -r ref -S data/binary/samples.g0.tsv -o output_binary_graph")
```
...
file_list <- list(
  ...
  binary_vcf_deconstruct = "/home/mbagarre/Bureau/stoat/output_binary_deconstruct/binary_table_vcf.tsv",
...
The main difference between **stoat vcf** and **stoat graph** lies in their input data and analysis approach:

* **Stoat VCF**: This mode takes a VCF (Variant Call Format) file as input, which contains genetic variant data for multiple samples. It analyzes the variants present in the VCF file to identify associations between the genetic variations present in the VCF and phenotypes (binary or quantitative). But doing the VCF can occure problem during the variant calling step, leading of errors, missing variants and significant difference between stoat vcf & stoat graph.

* **Stoat Graph**: This mode operates directly on a pangenome graph, which represents the genetic diversity of a population in a graph structure. Instead of relying on pre-called variants in a VCF file, it analyzes the graph to identify snarls (complex variant structures) and tests for associations between these snarls and phenotypes. This approach allows for a more comprehensive analysis of genetic variation.
...
In this simulation, we generated 200 samples with 200,000 SNPs and 100 genes. All variants were generated randomly, meaning no significant associations were introduced. The goal was to create a simple simulation to test stoat on eQTL data.
...
### Binary stoat VCF deconstruct

```{r binary-vcf-deconstruct, echo=FALSE}

# Execute verification for binary VCF
binary_output_deconstruct <- run_verify_truth(
  freq_file = file_list$binary_freq,
  pvalue_file = file_list$binary_vcf_deconstruct,
  paths_file = file_list$binary_snarl,
  flag = " -b ",
  output_dir = "../binary_output_deconstruct"
)

cat(binary_output_deconstruct, sep = "\n")

df_binary_vcf_deconstruct <- read_stoat(file_list$binary_vcf_deconstruct)

# Plots
histogram_plot(df=df_binary_vcf_deconstruct, subtitle="Binary [stoat vcf]", "P_FISHER", "P_CHI2")
qq_plot(df_binary_vcf_deconstruct, "P_FISHER", subtitle="Binary [stoat vcf]")
qq_plot(df_binary_vcf_deconstruct, "P_CHI2", subtitle="Binary [stoat vcf]")

#manhattan_plot(df_binary_vcf_deconstruct, subtitle="Binary [stoat vcf]", "P_CHI2", "P_FISHER", "P_ADJUSTED")
```

```{r show-plot-png-deconstruct, echo=FALSE, out.width="100%"}
knitr::include_graphics("/home/mbagarre/Bureau/stoat/tests/binary_output_deconstruct/pvalue_interactive.png")
knitr::include_graphics("/home/mbagarre/Bureau/stoat/tests/binary_output_deconstruct/10^-2_distribution_fn_tp.png")
# knitr::include_graphics("/home/mbagarre/Bureau/stoat/tests/binary_output_deconstruct/10^-2_distribution_false_positive.png")
# knitr::include_graphics("/home/mbagarre/Bureau/stoat/tests/binary_output_deconstruct/10^-2_distribution_true_negatives.png")

knitr::include_graphics("/home/mbagarre/Bureau/stoat/tests/binary_output_deconstruct/confusion_matrix_0.0_0.01.png")
knitr::include_graphics("/home/mbagarre/Bureau/stoat/tests/binary_output_deconstruct/confusion_matrix_0.0_1e-05.png")
knitr::include_graphics("/home/mbagarre/Bureau/stoat/tests/binary_output_deconstruct/confusion_matrix_0.0_1e-08.png")
```
...
Here we compare stoat vcf and stoat graph output for binary simulation by comparing each columns by matching SNARL.
If a column is different we count the number of different rows per column.
```{r binary-compare, echo=FALSE}
...
This change can be explained because the stoat vcf used VCF file computed with vg variant calling that can create bad results and stoat graph use the 'ref' haplotype as the correct haplotype, whereas in the simulation it is not. To test this theory we run stoat vcf with a deconstructed vcf (using vg deconstruct on the pangenome binary graph).

```{r binary-compare2, echo=FALSE}
# Compare each columns by matching SNARL
# Merge the data frames by SNARL
merged_df <- merge(df_binary_vcf_deconstruct, df_binary_graph, by = "SNARL", suffixes = c("_df1", "_df2"))

# Get the list of columns to compare (excluding SNARL)
columns_to_compare <- setdiff(names(df_binary_vcf_deconstruct), "SNARL")
...
# ---- Plots ----
histogram_plot(df=df_quantitative_vcf, subtitle="Quantitative [stoat vcf]", "P")#qq_plot(df_quantitative_vcf, "P", subtitle="Quantitative [stoat vcf]")
