...
/// A class for holding per-snarl data
...
class SnarlDataCollection {
    ...
    public:
        ...
        /// Run iteratee for all snarls with the given chr_name
        void for_each_snarl_in_chromosome(const std::string& chr_name, const std::function<void(const snarl_data_t& snarl_info)>& iteratee) const;
    ...
    private:
        ...
        std::unordered_map<std::string, std::vector<snarl_data_t>> chr_to_snarl_data;

    ////////////////////////////////////////// Private helper functions
    
    private:

        /// Go through all the snarls in the distance index and fill in a map from reference chromosome name to the snarl data on that path
        void get_all_snarl_data(const handlegraph::PathPositionHandleGraph& graph, const bdsg::SnarlDistanceIndex& distance_index,
                                                                                  size_t snarl_length_limit, size_t snarl_child_limit);
