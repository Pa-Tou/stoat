...
std::vector<size_t> AlleleBySampleMatrix<stoat::edge_t>::get_samples_on_path(const stoat::PathTraversal &path_trav) const {
    ...
    for (size_t i = 0; i < path_len - 1; ++i) {
        ...
        // make an key
        stoat::edge_t key(path[i], path[i + 1]);
        ...
        stoat::edge_t key_flipped = key.get_flipped();
...
template<>
void AlleleBySampleMatrix<stoat::edge_t>::load_vcf_chunk(stoat_vcf::VCFParser& vcf_parser, std::string &chr) {
                ...
                if (allele_num != -1) { // if the genotype wasn't .
                    ...
                    for (size_t node_i = 0 ; node_i < path.size()-1 ; node_i++) {
                        ...
                        if (path[node_i].get_node_id() != 0 && path[node_i+1].get_node_id() != 0) {
                            add_sample_key(stoat::edge_t(path[node_i], path[node_i+1]), hap_num);
                        }
                    }
                }
            }
...
template<>
void AlleleBySampleMatrix<stoat::node_traversal_t>::load_vcf_chunk(stoat_vcf::VCFParser& vcf_parser, std::string &chr) {
    // init the key matrix, allocating about 10 000 keys?
    clear_keys(10000);
        ...
        if (vcf_info.lv == 0 || vcf_parser.resolve_nested_calls) {
            ...
            for (size_t hap_num = 0 ; hap_num < vcf_parser.hap_count ; hap_num++) {
                ...
                if (allele_num != -1) { // if the genotype wasn't .
                    ...
                    for (size_t node_i = 0 ; node_i < path.size()-1 ; node_i++) {
                        if (path[node_i].get_node_id() != 0 && path[node_i+1].get_node_id() != 0) {
                            add_sample_key(path[node_i], hap_num);
                            add_sample_key(path[node_i+1], hap_num);
                        }
                    }
                }
            }
